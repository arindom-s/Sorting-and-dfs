async function partition(l, r, delay) {
  ...
  for (const j = l; j <= r - 1; j++) {
    ...
    const value = Number(blocks[j].childNodes[0].innerHTML);

    // compare value of two blocks
    if (value < pivot) {
      i++;
      const temp1 = blocks[i].style.height;
      const temp2 = blocks[i].childNodes[0].innerText;
  ...
  for (const k = 0; k < 15; k++) blocks[k].style.backgroundColor = "green";
  return i;
}
...
async function QuickSort(l, r, delay) {
  if (l < r) {
    ...
    const pivot_idx = await partition(l, r, delay);
